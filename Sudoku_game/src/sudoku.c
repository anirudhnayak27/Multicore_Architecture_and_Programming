...
int isValidGrid(grid_t grid) {
   ...
   int bx = (x / MINIGRIDSIZE) * MINIGRIDSIZE, by = (y / MINIGRIDSIZE) * MINIGRIDSIZE;
   int j, ox, oy;
   for (j = 0; j < SIZE; j++) {
      ox = j / MINIGRIDSIZE;
      oy = j % MINIGRIDSIZE;
...
int setLoneRangersBox(grid_t grid) {
   ...
   do {
      ...
      for(v = 0; v < SIZE; v++) {
         // vertical box index
         for(bi = 0; bi < SIZE; bi+=MINIGRIDSIZE) {
            // horizontal box index
            for(bj = 0; bj < SIZE; bj+=MINIGRIDSIZE) {
               count = 0;

               // look for a loner in this box
               for(i = bi; i < bi + MINIGRIDSIZE; i++) {
                  for(j = bj; j < bj + MINIGRIDSIZE; j++) {
...
int setTwinsBox(grid_t grid) {
   ...
   //box number
   for(bi = 0; bi < MINIGRIDSIZE; bi++) {
      for(bj = 0; bj < MINIGRIDSIZE; bj++) {
         // first cell with just two possible values
         for(j = 0; j < SIZE; j++) {
            idx1 = idx(bi + j / MINIGRIDSIZE, bj + j % MINIGRIDSIZE);
            if(valCount(grid->mask[idx1]) == 2) {
               // second cell with just two possible values
               for(k = j + 1; k < SIZE; k++) {

                  idx2 = idx(bi + k / MINIGRIDSIZE, bj + k % MINIGRIDSIZE);
                  if(grid->mask[idx1] == grid->mask[idx2]) {
                     // twin found. Remove the pair of possible values from
                     // all unset cells in box.
                     for(l = 0; l < SIZE; l++) {
                        if (l != j && l != k) {
                           idx3 = idx(bi + k / MINIGRIDSIZE, bj + k % MINIGRIDSIZE);
