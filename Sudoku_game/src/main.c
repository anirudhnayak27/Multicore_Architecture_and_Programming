...
int main(int argc, char *argv[]){
   if (argc<4){
      printf("Usage: ./sudoku <thread_count> <inputFile> <MINIGRIDSIZE>\n");
      exit(0);
   }
   if(atoi(argv[3]) <= 0){
      printf("Usage: MINIGRIDSIZE should be positive\n");
      exit(0);
   }
   int **originalGrid = readInput(argv[2]);
   int **gridToSolve = readInput(argv[2]);
   thread_count = atoi(argv[1]);
   size_t MINIGRIDSIZE = atoi(argv[3]);
   if (thread_count<=0){
