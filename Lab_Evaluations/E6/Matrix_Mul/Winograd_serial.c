...
void winograd_multiply_parallel(double A[N][N], double B[N][N], double C[N][N]) {
    double M[N/2][N/2], P[N/2][N/2], Q[N/2][N/2], R[N/2][N/2];
    double S[N/2][N/2], T[N/2][N/2], U[N/2][N/2], V[N/2][N/2];
    ...
    // Calculate C
    #pragma omp parallel for collapse(2) schedule(auto) num_threads(4)
    for (int i = 0; i < N/2; i++) {
        for (int j = 0; j < N/2; j++) {
            C[i][j] = M[i][j] + P[i][j] - S[i][N/2-1] + T[i][N/2-1];
            C[i][j+N/2] = R[i][j] + S[i][j] + U[i][N/2-1];
...
int main() {
    ...
    gettimeofday(&start, 0);
    winograd_multiply_parallel(A, B, C);
    gettimeofday(&end, 0);
